...
const TeamsPage = () => {
  ...
  const filteredTeamMembers = teamMembers.filter(member => {
    const nameMatch = filters.name === '' || member.name.toLowerCase().includes(filters.name.toLowerCase());
    const practiceAreaMatch = filters.practiceArea === '' || member.practiceArea.toLowerCase() === filters.practiceArea.toLowerCase();
    const positionMatch = filters.position === '' || member.position.toLowerCase() === filters.position.toLowerCase();
    
    return nameMatch && practiceAreaMatch && positionMatch;
  });
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {filteredTeamMembers.length > 0 ? (
            ...
            <p className="text-center col-span-full text-gray-600">No team members found matching the selected criteria.</p>
          )}
        </motion.div>
      </div>
    </div>
